...
class NMPCController:
    ...
    def solve(self, current_state, target_trajectory, target_speed=None):
        ...
        if target_speed is not None:
            v_target = target_speed  # 允许为0，传给cost function用于减速到停
        else:
        ...
        if self.last_u_seq is None:
            ...
            self.last_u_seq[:, 1] = max(v_target, 0.5)
        
        u0_seq = np.vstack([self.last_u_seq[1:, :], self.last_u_seq[-1, :]])
        u0_vec = u0_seq.flatten()
        
        # 边界约束: 如果目标速度 < 0.3, 允许 v_cmd 接近 0
        v_min_bound = 0.0 if v_target < 0.3 else max(self.min_v, 0.1)
        lb = []
        ub = []
        for _ in range(self.N_p):
            lb.extend([-self.max_steer, v_min_bound])
